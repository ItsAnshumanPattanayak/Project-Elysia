...
Memory candidates likewise cannot authorize persistence. Controlled types, evidence checks, secret and path rejection, sensitive-data confirmation, bounded fields and conversation ownership are enforced by the backend. Retrieved memories are explicitly untrusted prompt context and never sent to a cloud or embedding service. This mitigates prompt injection but cannot guarantee a local model will ignore every adversarial recollection. Anyone with local database access can read memory content; archive and delete controls are application lifecycle tools, not encrypted erasure.
