...
- **Memory:** conversation-scoped, typed recollection with normalized comparison text, confidence and importance, two-message provenance, a unique application key, lifecycle state, pin, lock and sensitive controls, supersession links, usage timestamps and safe audit metadata.
...
- No automatic summaries are produced. Relationship and memory derivations are separately auditable and idempotent.
...
Batch 5 migration `20260803_0003` replaces the original placeholder memory shape while migrating any retained rows. Importance becomes 0–100, confidence is 0–1, status is constrained to active, archived, superseded or reverted, and application keys are unique. Existing 1–5 importance is mapped in 20-point increments; former inactive rows become archived and permanent rows become pinned and locked.
